...
def load_image(foreground,  background):
...
for image_path in image_paths:
    if i == -1:
        directory.append(cv2.imread(image_paths[0]))
    else:
        foreground = directory[i]
        background = cv2.imread(image_path)
        directory.append(load_image(foreground, background))    
    i += 1 
    # cv2.imshow("processed", directory[i])
    # cv2.waitKey(0)

# print(len(directory))

fig, ax = plt.subplots()
ax.imshow(cv2.cvtColor(directory[0], cv2.COLOR_BGR2RGB)) 
